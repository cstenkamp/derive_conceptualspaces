...
from src.static.settings import SID_DATA_BASE, DEBUG, MONGO_URI
import src.fb_classifier
...
from src.fb_classifier.util.misc import get_all_configs
...
for pyfile in [join(path, name) for path, subdirs, files in os.walk(dirname(src.fb_classifier.__file__)) for name in files if splitext(name)[1] == ".py"]:
